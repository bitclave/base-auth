...
import { encodeQueryData } from '../core/Utils.js';
...
export default class WidgetView {
    constructor(
        ...
        this._applicationPublicKey = null;
    ...
    initialize() {
        ...
        return parentRpc.call('handshake', []).then(response => {
            ...
            return this._parentRpc.call('getApplicationPublicKey', []);
        }).then(response => {
            this._applicationPublicKey = response.value;
            return this._parentRpc.call('getUserPermissions', []);
        }).then(response => {
            this._userPermissions = response.value;
        });
    }
    ...
    _onClickLogin(event) {
        ...
        const popupQuery = encodeQueryData({
            application_public_key: this._applicationPublicKey,
            application_origin: this._parentOrigin,
            user_permissions: this._userPermissions.join(','),
        });
        const popup = window.open(
            [this._popupUrl, popupQuery].join('?'), this._popupName, "height=500, width=500"
        );
